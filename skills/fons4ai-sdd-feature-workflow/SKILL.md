...
| BUG、异常、错误行为、回归失败 | 转入 `fons4ai-bugfix-workflow`，由其判断 S0 Fast Fix 或标准模式 |
| 用户只是在已生成任务规划后确认实现 | 转入 `fons4ai-sdd-implement` |
| 用户只要求沉淀已验证事实 | 转入 `fons4ai-knowledge-summary` |
| 低风险已有内容变更（文案、样式、小配置、纯重命名、日志级别） | 转入 `fons4ai-sdd-change`，由其内置 S0 Fast Change 判断直接执行或升级 |
| 类型不清 | 先提出一个最高影响澄清问题 |

真正产生新业务能力的请求原则上不是 S0。分流时只说明推荐技能和原因，本编排技能不得亲自执行 S0 或修改业务代码；当前平台支持技能切换时，可在用户原请求已经明确授权修改的前提下切换到对应技能继续，不能切换时停止并给出路由结果。
