...
description: "Fons4AI 受控的 SDD 增量变更工作流。用于对已有 SDD 功能先澄清变更、分析影响、生成 CR，并把可执行增量任务追加到任务规划后停止等待实现确认。只有当用户明确指定该技能、明确要求迭代/变更已有功能，或启用 Fons4AI 路由且当前意图匹配已有功能变更时使用；全新需求开发优先使用 fons4ai-sdd-feature-workflow。"
---
...
- Required: 目标既有功能目录，或足以唯一定位既有 SDD 功能的变更请求。
...
- Entry gates: 满足触发门禁，且能唯一定位目标功能目录；多候选时必须先询问用户选择。
- Required source artifacts: 必须读取目标功能目录中的需求、设计、任务、changes 和 reports；已有 CR 或未完成任务必须先读取。
- Required repository facts: 必须对变更影响的模块、接口、数据结构、测试和规则做定向搜索；结构变更必须有 DDL/schema 证据，证据不足时标记待确认并阻塞。

### Outputs

- May create or update: `spec/features/<yyyymmdd>/changes/CR-xxx.md`、受影响的 SDD 产物和任务规划中的增量任务。
- Must not create or update: 业务代码、知识库正文、知识同步任务和直接可执行的生产 DDL。
- Output location: 目标 feature 目录及其 `changes/` 子目录。

### Exit Criteria

- Success: CR 已生成或更新，受影响 SDD 产物已同步，增量任务已追加到任务规划，实施状态保持 `pending`，且可用校验通过。
- Blocked: 目标功能不唯一、变更意图不清、源产物互相冲突、DDL/schema 证据缺失、影响范围超过 70% 或需要用户裁决。
...
2. 用户明确要求迭代、调整、变更或追加已有 SDD 功能。
3. 当前仓库作用域内存在启用路由的 `AGENTS.md`，且用户当前意图明确匹配已有功能变更。
