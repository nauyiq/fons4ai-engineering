...
- Status rule: 反馈单 `Status` 必须反映证据成熟度；存在占位路径、待确认事实、待脱敏内容或不确定归因时只能写 `Draft`，不得写 `Ready`。
...
- Next skill: 默认无自动下游；如反馈单确认是通用上游问题，可建议进入 Loop Phase 1，在 `fons4ai-engineering` 中创建 Loop 改进记录。
- Required handoff fields: 反馈单路径、问题分类、关联技能、是否建议回流上游、建议修改位置、未确认项、是否建议进入 Loop Phase 1。
...
- `Ready` 状态不得包含 `<date>`、`<feature>`、`<报告>` 等占位来源；占位来源对应的证据状态不得写 `已验证`。
- 只有单项目证据时，可以写“可抽象为上游规则”或“建议回流观察”，不得写成“具有跨项目通用性”或 `CROSS_PROJECT_REPEAT` 已确认。
...
6. 生成前自检。
   - 检查 `Status`：若存在占位路径、待确认事实、待脱敏项或未关闭不确定项，必须写 `Draft`。
   - 检查证据清单：每条 `L2`/`L3` 且 `已验证` 的证据必须能定位到报告、脚本输出、技能文本、模板文本或用户确认记录。
   - 检查跨项目表述：若证据清单写“当前仅基于一个试点项目”或“尚未形成多项目重复证据”，归因说明不得写“具有跨项目通用性”。
   - 检查建议回流：证据不足但方向有价值时写 `建议回流：待观察`，不得写成已确认上游改进结论。
...
- 不把生成反馈单等同于完成上游修复。
- 不写入 `.specify/` 目录。
- 如果反馈单 `Ready` 且建议回流上游，结尾给出 Loop Phase 1 handoff：建议创建 `spec/reports/loop-improvements/<yyyymmdd>-<问题简述>-loop改进记录.md`，使用 `templates/loop-improvement-record-template.md` 记录修改清单、验证结果和全局同步状态。
