...
| 技能类型 | Evidence 强度 | 说明 |
| --- | --- | --- |
| 实现类 | 按风险分级 | S0 记录最小充分验证；S1/S2 完成必须有完整 L3 Evidence |
| BUG 修复类 | 按风险分级 | S0 Fast Fix 在对话记录失败信号、根因和验证；标准模式使用完整 Evidence 与报告 |
...
S0 不作为独立技能，也不依赖项目先生成规则。BUG 技能和已有内容变更技能必须在 frontmatter 与正文中内置相同语义的默认判断；项目规则可以收紧，不得默认放宽数据、契约、安全和关键业务硬门禁。

满足以下任一情况即不得使用 S0：业务规则/AC/状态流转变化、公共契约变化、数据结构或语义变化、权限安全或关键业务影响、跨核心模块/服务、迁移或高成本回滚、超过五个手工文件、预期结果不明确、无法快速聚焦验证。

S0 在用户明确要求修改时直接执行，不生成 CR、任务规划或必需报告，不重复请求实现确认；只读请求不得写文件，执行中范围扩大必须升级。
...
`agents/openai.yaml` 是 OpenAI/Codex 的适配文件名；其中的 `adapter.platform` 应填写运行平台标识 `codex`。文件名和运行平台标识不是同一概念。
